...
let currentUsername = localStorage.getItem('app_username') || 'User';
...
function enableButtons() {
  ...
  if (btnGenCode) {
    ...
    btnGenCode.textContent = "Generate token code";
  }
  if (btnGenToken) {
...
function checkUsernameChangeState() {
  if (hasChangedUsername && btnToName) {
...
function handleGeneration(type) {
  ...
  if (!isUserWhitelisted() && (now - lastGeneratedTime < COOLDOWN_TIME)) return;
